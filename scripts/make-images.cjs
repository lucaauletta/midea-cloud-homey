...
function draw(w,h){return png(w,h,(x,y)=>{
  const u=x/w,v=y/h; const bg=[26+40*v,115+30*v,232-40*v];
  const bx0=0.14,bx1=0.86,by0=0.30,by1=0.62; const r=0.06*Math.min(1,w/h);
  const inBox=u>bx0&&u<bx1&&v>by0&&v<by1; if(!inBox) {
    for(const cx of [0.32,0.5,0.68]){ if(Math.abs(u-cx)<0.012 && v>0.68 && v<0.84) return [255,255,255]; }
    return bg.map(Math.round); }
  const louver=Math.abs(v-0.52)<0.015 && u>bx0+0.05 && u<bx1-0.05; if(louver) return [40,60,90];
  return [255,255,255]; }); }
const out=(p,w,h)=>{fs.mkdirSync(path.dirname(p),{recursive:true});fs.writeFileSync(p,draw(w,h));console.log('wrote',p,w+'x'+h);};
...
out('drivers/ac/assets/images/small.png',75,75); out('drivers/ac/assets/images/large.png',500,500); out('drivers/ac/assets/images/xlarge.png',1000,1000);
